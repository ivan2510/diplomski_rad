...
def detect(
	...
	if weights_file_path.endswith('.pt'):  # pytorch format
		if weights_file_path.endswith('weights/yolov3.pt') and not os.path.isfile(weights_file_path):
			os.system('wget https://storage.googleapis.com/ultralytics/yolov3-tiny.pt -O ' + weights_file_path)
	...
	images_path = "/home/ivan/Desktop/yolov3-master (copy)/data/samples1"
	...
	# Bounding-box colors
	color_list = [[255, 0, 0] for _ in range(len(classes))]
	...
	for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
		...
		if detections is not None and detections[0] is not None:
			...
			for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
				...
				#print(img.shape[1])
				y1 = y1.round().item() #(y1 // 2).round().item() #* img.shape[0]).round().item()
				x1 = x1.round().item() #(x1 // 2).round().item() #* img.shape[1]).round().item()
				...
				if save_images:
					...
					label = '%.2f' % (conf) #'%s %.2f' % (classes[int(cls_pred)], conf)
					color = bbox_colors[int(numpy.where(unique_classes == int(cls_pred))[0])]
					#image_bboxs.append([x1+x1_coord[img_cnt], y1, x2+x1_coord[img_cnt], y2])
					#print("x1: " + str(x1+x1_coord[img_cnt])")
					#image_confs.append(label)
					#bbox_color.append(color)
					plot_one_box([x1, y1, x2, y2], img, label=label, color=color)
			#if video:
			#	img_cnt+=1
			#	images_frame.append(img)

			#if img_cnt==6: #len(images_frame)==6:
			#	img_cnt = 0
			#	result = stitching(images_frame)
			#	image_bboxs = non_max_suppression_slow(numpy.array(image_bboxs))
			#	image_bboxs = list(image_bboxs)
			#	for i in range(len(image_bboxs)):
			#		plot_one_box(image_bboxs[i], result, label=image_confs[i], color=bbox_color[i])
...
if __name__ == '__main__':
	...
	parser.add_argument('--image-folder', type=str, default='data/samples1', help='path to images')
	...
	parser.add_argument('--img-size', type=tuple, default=(3260,570), help='size of each image dimension')
	parser.add_argument('--video', type=bool, default=False, help='show if the video is in the images path')
