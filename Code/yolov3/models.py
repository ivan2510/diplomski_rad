...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            yolo_layer = YOLOLayer(anchors, num_classes, (img_height, img_width), anchor_idxs, cfg=hyperparams['cfg'])
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, nC, img_dim, anchor_idxs, cfg):
        ...
        self.img_dim = img_dim  # from hyperparams in cfg file, NOT from parser, tuple (height, width)
        ...
        # Build anchor grids
        """Dodano"""
        nG_h = int(np.ceil(self.img_dim[0] / stride))  # number grid points for height
        nG_w = int(np.ceil(self.img_dim[1] / stride))  # number grid points for width
        self.grid_x = torch.arange(nG_w).repeat(nG_h, 1).view([1, 1, nG_h, nG_w]).float()
        #print(self.grid_x.shape)
        self.grid_y = torch.arange(nG_h).repeat(nG_w, 1).t().view([1, 1, nG_h, nG_w]).float()
    ...
    def forward(self, p, targets=None, batch_report=False, var=None):
        ...
        nG_h = p.shape[2]  # number of grid points height
        #print("Forward height:" + str(nG_h))
        nG_w = p.shape[3]  #number of grid points width
        #print("Forward width:" + str(nG_w))
        #print("P:" + str(p.shape))
        ...
        p = p.view(bs, self.nA, self.bbox_attrs, nG_h, nG_w).permute(0, 1, 3, 4, 2).contiguous()  # prediction
        #print("P after: " + str(p.shape))
        # Training
        if targets is not None:
            ...
            if batch_report:
                ...
                gx = x.data + self.grid_x[:, :, :nG_h, :nG_w]
                gy = y.data + self.grid_y[:, :, :nG_h, :nG_w]
                p_boxes = torch.stack((gx - width / 2,
                                       gy - height / 2,
                                       gx + width / 2,
                                       gy + height / 2), 4)  # x1y1x2y2

            """Dodati sirinu i visinu u build targets ako bude radia yolo s novim dimenzijama"""
            tx, ty, tw, th, mask, tcls, TP, FP, FN, TC = \
                build_targets(p_boxes, p_conf, p_cls, targets, self.anchor_wh, self.nA, self.nC, (nG_w, nG_h), batch_report)
            ...
            #print("P0: " + str(p[...,0].shape))
            #print("X shape:" + str(self.grid_x.shape))
            #print("P0: " + str(p[...,1].shape))
