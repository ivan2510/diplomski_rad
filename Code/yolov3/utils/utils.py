...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):  # Plots one bounding box on image img
    ...
    cv2.rectangle(img, c1, c2, color, thickness=tl-3)
    if label:
        tf = max(tl - 3, 1)  # font thickness
        #print("tf: " + str(tf))
        ...
        cv2.putText(img, label, (c1[0], c1[1] - 4), 0, tl / 7, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
