...
class load_images_and_labels():  # for training
    def __init__(self, path, batch_size=1, img_size=608, multi_scale=False, augment=False, test=False):
        self.path = path
        ...
        #                    self.img_files]
        if test:
          label_path = "/content/drive/My Drive/label_paths_with_boxes2.txt"
        else:
          label_path = "/content/drive/My Drive/label_paths_with_boxes.txt"
         with open(label_path, 'r') as f:
            self.label_files = f.readlines()
        self.label_files = [path.replace('\n', '') for path in self.label_files]
    ...
    def __next__(self):
        ...
        for index, files_index in enumerate(range(ia, ib)):
            ...
            if os.path.isfile(label_path):
                ...
                labels = labels0.copy()
                labels[:, 1] = ratio*labels[:, 1] + padw # * w * (labels0[:, 1] - labels0[:, 3] / 2)
                labels[:, 2] = ratio*labels[:, 2] + padh #ratio * h * (labels0[:, 2] - labels0[:, 4] / 2)
                labels[:, 3] = ratio*labels[:, 3] + padw #ratio * w * (labels0[:, 1] + labels0[:, 3] / 2) +
                labels[:, 4] = ratio*labels[:, 4] + padh #ratio * h * (labels0[:, 2] + labels0[:, 4] / 2) +
            else:
                labels = np.array([])
            ...
            if nL > 0:
                ...
                if self.height == self.width:
                  ...
                  labels[:, 1] = labels[:, 1] / width
                  labels[:, 2] = labels[:, 2] / height
                  labels[:, 3] = labels[:, 3] / width
                  labels[:, 4] = labels[:, 4] / height
